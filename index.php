...
#game-bg > *:not(#game-hud):not(#chapter-card):not(#end-card):not(#flash-overlay){
...
.bg-mansion{
  background:
    linear-gradient(rgba(8,2,16,.6),rgba(4,1,10,.82)),
...
.bg-interrogation{
  background:
    radial-gradient(ellipse 28% 55% at 50% 32%,rgba(230,200,110,.13) 0%,rgba(0,0,0,.88) 65%),
...
.bg-night{
  background:
    linear-gradient(rgba(2,5,18,.55),rgba(3,6,12,.75)),
...
.bg-warehouse{
  background:
    linear-gradient(rgba(0,0,0,.68),rgba(0,0,0,.80)),
...
.bg-office{
  background:
    linear-gradient(rgba(4,6,16,.65),rgba(2,4,14,.82)),
...
.bg-blood{
  background:
    radial-gradient(ellipse at center,rgba(60,0,8,.7) 0%,rgba(0,0,0,.9) 70%),
    url('https://images.unsplash.com/photo-1558618666-fcd25c85cd64?auto=format&fit=crop&w=1920&q=80')
    center/cover no-repeat;
}
.bg-escape{
  background:
    linear-gradient(rgba(2,8,4,.58),rgba(3,10,5,.78)),
...
function playAmbientChord() {
  ...
  shuffled.forEach((freq, i) => {
    ...
    if (i === 0) {
      setTimeout(() => {
        const tb = document.getElementById('game-textbox');
        tb.style.transition = 'border-top-color .3s';
        tb.style.borderTopColor = 'rgba(255,255,255,.14)';
        setTimeout(() => { tb.style.borderTopColor = 'rgba(255,255,255,.06)'; }, 400);
      }, offset * 1000);
    }
  });
}
...
let paused = false, pausedText = '', pausedIdx = 0, pausedChoices = null;
...
function togglePause() {
  ...
  if (paused) {
    ...
    if (musicOn) { clearInterval(musicInterval); }
  } else {
    if (musicOn) { playAmbientChord(); musicInterval = setInterval(playAmbientChord, 7000); }
    if (typing) resumeTyping();
  }
}
